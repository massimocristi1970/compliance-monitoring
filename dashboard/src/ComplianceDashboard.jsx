...
const ComplianceDashboard = () => {
  ...
  const saveComplianceDataToGitHub = async (dataToSave) => {
    ...
    try {
      ...
      // We remove the alert from here to stop double-alerts
    } catch (error) {
  ...
  const saveCheckNotes = async (checkRef, notes) => {
    ...
    try {
      ...
      // Pass the new data directly to the save function
  ...
  const handleFileUpload = async (checkRef, files) => {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        const filePath = `data/${selectedYear}/${months[
          selectedMonth - 1
        ].toLowerCase()}/check-${checkRef}/${file.name}`;
      ...
      alert(
        `✅ Files uploaded successfully by ${
          user.name || user.login
        }:\n${uploadedFiles
          .map((f) => f.name)
          .join(
        ...
        {!isAuthenticated && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            ...
            <p className="text-blue-700 text-sm mt-1">
              Login with GitHub to upload files, save changes, and access live
              compliance data. You can still view and generate reports without
              authentication.
            </p>
          </div>
        )}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {isAuthenticated ? (
                            ...
                            <button
                              onClick={() =>
                                alert(
                                  "Please authenticate with GitHub to upload files."
                                )
                              }
                ...
                {!isAuthenticated ? (
                  <div className="text-center">
                    ...
                    <p className="text-gray-600 mb-4">
                      You need to authenticate with GitHub to upload files to
                      the repository.
                    </p>
                    <button
                      onClick={login}
                  ...
                  <>
                    <div className="mb-4">
                      <p className="text-sm text-gray-600 mb-3">
                        Files will be uploaded to GitHub repository by{" "}
                        {user.name || user.login}:
                      </p>
