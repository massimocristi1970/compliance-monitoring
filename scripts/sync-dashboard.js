...
async function syncDashboardData() {
  ...
  const dataDir = "../data";
  const dashboardDataDir = "../dashboard/public/data";
  ...
  try {
    ...
    if (!(await fs.pathExists(dataDir))) {
      console.log(
        "📁 No data directory found, creating empty dashboard data..."
      );
      await fs.writeJson(
      ...
      for (const month of months) {
        ...
        for (const check of checks) {
          ...
          try {
            ...
            console.warn(
              `⚠️  Could not read metadata for ${checkPath}:`,
              error.message
            );
          }
        }
    ...
    await fs.writeJson(path.join(dashboardDataDir, "summary.json"), summary, {
      spaces: 2,
    });

    console.log(
      `✅ Dashboard sync complete! Updated ${complianceData.length} checks.`
    );
